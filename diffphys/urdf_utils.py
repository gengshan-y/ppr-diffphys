            ...
            for jdx in range(len(path) - 1):
                ...
                if joint.name in name2joints_idx.keys():
                    ...
                    for kdx in range(jdx + 1, len(path) - 1):
                        # find the next joint
                        next_joint = urdf._G.get_edge_data(path[kdx], path[kdx + 1])[
                            "joint"
                        ]
...
def render_robot(robot, save_path, cfg=None, use_collision=False):
    ...
    cam = pyrender.OrthographicCamera(xmag=0.5, ymag=0.5)
    cam_pose = np.eye(4)
    cam_pose[:3, :3] = cv2.Rodrigues(np.asarray([0.0, -np.pi / 4 * 2, 0.0]))[0]
    cam_pose[0, 3] = -1
    cam_node = scene.add(cam, pose=cam_pose)
    direc_l = pyrender.DirectionalLight(color=np.ones(3), intensity=6.0)
    light_pose = np.eye(4)
    light_pose[:3, :3] = cv2.Rodrigues(
        np.asarray([-np.pi / 8 * 3.0, -np.pi / 8 * 5, 0.0])
    )[0]
    light_pose[0, 3] = -1
