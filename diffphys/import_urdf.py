...
def parse_urdf(
    ...
    for joint in robot.joints:
        ...
        if joint.name[-2:]=='_R':
            type = wp.sim.JOINT_COMPOUND # ball joints
            joint.child = joint.child[:-2] + '_Y'
        elif joint.name[-2:]=='_P' or joint.name[-2:]=='_Y': 
            continue
        ...
        if type==wp.sim.JOINT_COMPOUND: # ball joints
            joint_stiffness = []
            joint_damping = []
            joint_lower = []
            joint_upper = []
            ...
            for i in range(3):
                ...
                joint_stiffness.append(stiffness)
                joint_damping.append(damping)
                if i==0:
                    joint_axis.append(wp.normalize(np.asarray([1,0,0])))
                elif i==1:
                    joint_axis.append(wp.normalize(np.asarray([0,1,0])))
                elif i==2:
                    joint_axis.append(wp.normalize(np.asarray([0,0,1])))
