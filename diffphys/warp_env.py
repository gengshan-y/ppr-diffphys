...
class phys_model(nn.Module):
    def __init__(self, opts, dataloader, dt=5e-4, use_dr=False, device="cuda"):
        ...
        if self.use_dr:
            ...
            self.delta_root_mlp = WarpRootMLP(
                self.nerf_root_rts, self.delta_joint_est_mlp, self.bg_rts
            )
    ...
    def pred_est_q(self, steps_fr):
        out, obj2view = pred_est_q(
            steps_fr, self.nerf_root_rts, self.nerf_body_rts, self.bg_rts
        )
        return out, obj2view
    ...
    def override_states(self):
        self.delta_root_mlp.override_states(
            self.nerf_root_rts, self.nerf_body_rts, self.bg_rts
        )
    ...
    def override_states_inv(self):
        self.delta_root_mlp.override_states_inv(
            self.nerf_root_rts, self.nerf_body_rts, self.bg_rts
        )
