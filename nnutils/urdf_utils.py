...
def compute_bone_from_joint(model, is_init, vid=None):
    ...
    sim3_exp = model.robot.sim3.to(device).exp()
    ...
    for link in urdf._reverse_topo:
        ...
        if len(path)>1:
            ...
            if joint.name not in urdf.name2query_idx:
                continue
        if is_init:
            if len(link.visuals)>0 and (not model.opts.unit_link):
                ...
                link_scale = link_scale * sim3_exp[7:]
                ...
                link_scale = torch.Tensor([1,1,1]).to(device)
                link_scale = link_scale * sim3_exp[7:]
                link_center = fk[idx][:3,3]
                #else: link_center = model.bones[idx,:3]
                for joint_child in urdf.joints:
                    if joint_child.parent == link.name and idx>0: 
                        jdx = urdf.name2joints_idx[joint_child.name] + 1
                        link_center = (fk[idx][:3,3] + fk[jdx][:3,3])/2
                        #link_scale = torch.Tensor([1,1,1]).to(device) * \
                        #            (fk[idx][:3,3] - fk[jdx][:3,3]).norm(2,-1)
                        #link_scale = link_scale * 5
                        break
                link_scale *= 2
...
def angle_to_rts(robot, joints, angles, sim3):
    ...
    #fk0 = link_fk_old(robot, joints, joint_cfg, sim3)
    #assert((fk-fk0).abs().mean()==0)
    return fk
...
def link_fk_old(urdf, joints, joint_cfg, sim3):
    """
    ...
    for link in urdf._reverse_topo: # go from root
...
def link_fk(urdf, joints, joint_cfg, sim3):
    ...
    return -1,B,4,4 bones/links
    """
    #TODO turn joint axis to cuda
    if not hasattr(urdf, 'device') or sim3.device!=urdf.device:
        urdf.device = sim3.device
        joint_axis = []
        joint_origin=[]
        for idx,joint in enumerate(urdf.joints):
            joint_axis.append(torch.Tensor(joint.axis).to(sim3.device).view(1,3))
            joint_origin.append(torch.Tensor(joint.origin).to(sim3.device))
        urdf.joint_axis = joint_axis
        urdf.joint_origin = joint_origin
    ...
    for idx,joint in enumerate(urdf.joints):
        ...
        if joint.name in joint_cfg.keys():
            ...
            angle = zero_mat
        axis = urdf.joint_axis[idx]
        se3_log = torch.cat([zero_mat, angle*axis],1)
        se3_log_list.append(se3_log)
        se3_log_kv[joint.name] = idx

        # get joint center
        origin = urdf.joint_origin[idx]
        origin = origin.view(1,4,4).repeat(bs,1,1) # bs, k
        if joint.name in urdf.name2joints_idx.keys():
            origin[...,:3,3] = joints[:,urdf.name2joints_idx[joint.name]]
        joint_center_list.append(origin)
    ...
    #torch.cuda.synchronize()
    #print('(((traverse time:%.6f'%(time.time()-start_time))
...
def visualize_joints(model, query_time=None, robot_save_path="tmp/robot.jpg"):
    ...
    if query_time is not None:
        ...
        vid = None
    sim3 = model.nerf_body_rts.compute_sim3(vid)[0]
...
def query_kps(model, query_time, query_name='kp_links'):
    """
    query_time: T
    """
    with torch.no_grad():
    ...
    for angles_t in angles:
        ...
        kp = []
        for kp_link, kp_T in kp_dict.items():
            if kp_link.name in getattr(model.robot.urdf, query_name):
                kp.append(kp_T[:,3]) 
        kp = np.stack(kp,0) # k,4
        kps.append( kp )
