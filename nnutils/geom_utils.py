...
def evaluate_mlp(model, xyz_input, embed_xyz=None, dir_embedded=None,
                chunk=32*1024, 
                xyz=None,
    ...
    """
    B,nbins,_ = xyz_input.shape
    out_chunks = []
    for i in range(0, B, chunk):
        embedded = xyz_input[i:i+chunk]
        if embed_xyz is not None:
            # apply video-specific scaling for bg points
            if hasattr(model, 'scale'):
                embedded = embedded / model.scale[vidid[...,None]].exp()
...
def skinning(xyz, embedding_xyz, bones, 
                    pose_code,  nerf_skin, vidid=None):
    """
    ...
    if nerf_skin is not None and nerf_skin.skin_type == 'cmlp':
        ...
        skin,dskin = gauss_mlp_skinning(xyz, embedding_xyz, bones, 
                    pose_code,  nerf_skin, vidid=vidid)
...
def gauss_mlp_skinning(xyz, embedding_xyz, bones, 
    ...
    if nerf_skin is not None:
        ...
        if nerf_skin.skin_type=='mlp':
            dskin = mlp_skinning(nerf_skin, pose_code, xyz_embedded)
        elif nerf_skin.skin_type=='condmlp':
            dskin = mlp_skinning(nerf_skin, pose_code, xyz_embedded, vidid=vidid)
        skin += dskin
    return skin, dskin
...
def mlp_skinning(mlp, code, pts_embed, vidid=None):
    """
    ...
    if mlp is None:
        ...
        dskin = evaluate_mlp(mlp, pts_embed, code=code, chunk=8*1024, vidid=vidid)
    return dskin
...
def chunk_rays(rays,start,delta):
    ...
    for k,v in rays.items():
        if torch.is_tensor(v):
            ...
            rays_chunk['bg'] = chunk_rays(rays['bg'],start,delta)
    return rays_chunk
...
def reinit_bones(model, mesh, num_bones):
    ...
    if model.opts.unit_link:
        bones,_ = zero_to_rest_bone(model, bones, inverse=True, is_init=True)
    else:
        bones,_ = zero_to_rest_bone(model, bones, inverse=True, is_init=False)
    model.bones.data = bones
    model.nerf_models['bones'] = model.bones
    return

def zero_to_rest_bone(model, bones_rst, inverse=False, vid=None, is_init=False):
    """
    Returns 
    ...
    if model.opts.pre_skel!="":
        ...
        bones = compute_bone_from_joint(model, is_init=is_init, vid=vid)
...
def warp_bw(opts, model, rt_dict, query_xyz_chunk, embedid):
    ...
    if opts.lbs:
        ...
        skin_backward,_ = skinning(query_xyz_chunk, model.embedding_xyz,
                   bones_dfm, time_embedded, nerf_skin, vidid=vidid)
    ...
    if opts.lbs:
        ...
        if opts.mlp_deform:
            ...
            pts_can = model.mlp_deform.forward(model.dfm_code(query_time)[:,None], 
                                        pts_can[:,:,None])[:,:,0]
        vidid,_ = fid_reindex(embedid, model.num_vid, model.data_offset)
        if vidid.dim()!=pts_can.dim()-1:
            vidid =  vidid[None].expand(pts_can.shape[:-1])
        vidid = vidid.to(pts_can.device)
        skin_forward,_ = skinning(pts_can, model.embedding_xyz, bones_rst, 
                            rest_pose_code, nerf_skin, vidid=vidid)
...
def get_near_far(near_far, vars_np, tol_fac=1.5, pts=None, sid=None,eid=None,beta=0):
    """
    pts:        point coordinate N,3
    ...
    tol_fac     tolerance factor
    """
    old_near_far = near_far.clone()[sid:eid]
    new_near_far = near_far.clone()[sid:eid]
    if pts is None:
    ...
    rtk = torch.Tensor(vars_np['rtk']).to(device)[sid:eid]
    idk = torch.Tensor(vars_np['idk']).to(device)[sid:eid]
    near_far_proj = get_near_far_sub(pts, rtk, tol_fac=tol_fac)
    ...
    ###
    new_near_far[idk==1] = near_far_proj[idk==1]

    new_near_far = old_near_far*beta  + new_near_far * (1-beta)
    near_far[sid:eid] = new_near_far
    return near_far
...
def pts_to_view(pts, rtk, device):
    ...
    for i in range(0,M,chunk):
        ...
        pts_sub = obj_to_cam(pts_sub,  rtk_sub[:,:3,:3], 
                                       rtk_sub[:,:3,3])
        if rtk_sub.shape[-2]==4: # has intrinsics params
...
def fit_plane_contact(can_to_env, kps, high_pt=None, meshbg=None):
    ...
    plane1 = pyrsc.Plane()
    if meshbg is not None and len(meshbg.vertices)>400:
...
def eval_fgbg_scale(bg2fg_scale, cams, bgcams, kps, verts):
    ...
    if nkp>1:
        ...
        loss = raw_cd.mean() # make sure top 50% kps are on ground
...
def optimize_scale(bgcams_np, cams_np, kps_np, mesh, 
                init_scale=0, debug=False, high_pt=None, use_mesh_plane=True, force_above=False):
    """
    output: scale applied to bg such that it aligns with fg (case1: slighter bigger; case2: mean)
            bg2world in the same scale as bg
            bgcams in the scale of fg (case 2)
    """
    mesh = mesh.copy()
    # ransac scheme?
    ...
    for it,val in enumerate(np.linspace(-3,3,200)): # 5: 148x, 3: 20x
        ...
        kps_out_all.append(kps_out_sub)
    val_idx = torch.stack(loss_queried,0).argmin()
    bg2fg_scale.data[:] = val_input[val_idx]
    kps_out = kps_out_all[val_idx]
    
    # reduce after first init
    bg_high_pt = cams_np.numpy() @ high_pt # canonical to view
    bg_high_pt[:,:3] *= (-bg2fg_scale).exp().detach().cpu().numpy() # fg2bg scaling
    bg_high_pt = np.linalg.inv(bgcams_np) @ bg_high_pt # view to bg
    ...
    if len(verts) == 0: 
        print('no bg vertices after initial fitting')
        return [1], bgcams_np, np.eye(4), np.eye(3)
    ...
    # fit plane
    bgcams_np_input = bgcams_np.clone()
    ...
    if use_mesh_plane:
        ...
        bg2world,xy2xz,plane = fit_plane_contact(can_to_env, kps_np, high_pt=high_pt,
                                            meshbg=meshbg_scaled)
    else:
        # use foot plane for quadruqed since 4 => plane more robust
        bg2world,xy2xz,plane = fit_plane_contact(can_to_env, kps_np, high_pt=high_pt)

    #TODO this bg2world is a function of scale. Change it to scale-invariant. Multiple scale when being used.
    bg2world[:3,3] /= scale
    plane[:3] /= scale # input scale

    #TODO rescale such that the lowest point is above the ground
    # algorithm: increase scale until the distance became zeros
    if force_above:
        scale = force_above_plane(kps_np, cams_np, bgcams_np_input.cpu().numpy(), plane, scale)

    if debug:
        from utils.io import vis_kps
        bg2world_scaled = bg2world.copy()
        bg2world_scaled[:3,3] *= scale
        kps = bg2world_scaled @ can_to_env @ kps_np
        vis_kps(kps, 'tmp/kps.obj')
    return scale, bgcams_np, bg2world, xy2xz
        
def force_above_plane(kps_np, cams_np, bgcams_np, plane, scale):
    """
    plane: 6, center, normal (input bg geometry)
    bgcams_np: tx4x4 (input bg cam)
    cams_np: tx4x4
    kps_np: tx4x?
    """
    maxdis=1
    plane_d = -plane[:3].dot(plane[3:6])
    kps_cam = cams_np@kps_np
    bgcams_np_inv_const = np.linalg.inv(bgcams_np)
    counter=1000
    while maxdis>0 and counter>0:
        #bgcams_np_inv = bgcams_np_inv_const.copy()
        #bgcams_np_inv[:,:3,3] *= scale # align to fg
        #scaled_kps_bg = bgcams_np_inv @ kps_cam 
        #scaled_kps_bg = np.transpose(scaled_kps_bg[:,:3],[0,2,1]).reshape((-1,3))
        #maxdis=(plane[3:6][None]*(scaled_kps_bg)).sum(-1)+plane_d * scale # align to fg
        
        kps_cam_copy = kps_cam.copy()
        kps_cam_copy[:,:3] /= scale # align to bg
        scaled_kps_bg = bgcams_np_inv_const @ kps_cam_copy
        scaled_kps_bg = np.transpose(scaled_kps_bg[:,:3],[0,2,1]).reshape((-1,3))
        maxdis=(plane[3:6][None]*(scaled_kps_bg)).sum(-1)+plane_d # align to fg
    ...
    if len(mesh.vertices)>0:
        ...
        if is_eval:
            ...
            try:
                ...
                #                        ms.current_mesh().face_matrix())
                vw, fw = pcu.make_mesh_watertight(mesh.vertices, mesh.faces, 10000)
...
def extract_mesh(model,chunk,grid_size,
    ...
    if mesh_dict_in is None:
                ...
                for i in range(0, bs_pts, chunk):
                    ...
                    if opts.nerf_vis:
                        ...
                        if vidid is not None:
                            ...
                            vidid_chunk = vidid * torch.ones(xyz_embedded.shape[:-1]).to(device)
                            vidid_chunk = vidid_chunk.long()
                            if hasattr(model.nerf_vis, 'vid_code'):
        ...
        if hasattr(model, "bg2fg_scale"):
            mesh.vertices *= model.bg2fg_scale[int(vidid)].exp().cpu().numpy()
            mesh = mesh.copy()
...
def interpolate_linear(y, t):
    # Compute the integer and decimal parts of t
    t_floor = torch.floor(t).long()
    t_ceil = torch.ceil(t).long()
    t_ceil.clamp_(max=len(y)-1)
    t_frac = t - t_floor.float()

    # Use integer parts to index y
    y_floor = torch.index_select(y, 0, t_floor)
    y_ceil = torch.index_select(y, 0, t_ceil)

    # Linear interpolation
    y_interpolated = (1 - t_frac[...,None]) * y_floor + t_frac[...,None] * y_ceil

    return y_interpolated

def slerp(val, low, high, epsilon=1e-6):
    # Normalize input quaternions.
    low_norm = low / torch.norm(low, dim=1, keepdim=True)
    high_norm = high / torch.norm(high, dim=1, keepdim=True)
    
    # Compute cosine of angle between quaternions.
    cos_angle = torch.clamp((low_norm * high_norm).sum(dim=1), -1.0, 1.0)
    omega = torch.acos(cos_angle)

    so = torch.sin(omega)
    t1 = torch.sin((1.0-val)*omega) / (so+epsilon)
    t2 = torch.sin(val*omega)/ (so+epsilon)
    return t1.unsqueeze(-1) * low + t2.unsqueeze(-1) * high

def interpolate_slerp(y, t):
    # Compute the integer and decimal parts of t
    t_floor = torch.floor(t).long()
    t_ceil = torch.ceil(t).long()
    t_ceil.clamp_(max=len(y)-1)
    ...
    y_floor_norm = y_floor / y_floor.norm(dim=1).unsqueeze(1)
    y_ceil_norm = y_ceil / y_ceil.norm(dim=1).unsqueeze(1)

    # Compute interpolated quaternion
    y_interpolated = slerp(t_frac, y_floor_norm, y_ceil_norm)
    y_interpolated_norm = y_interpolated / y_interpolated.norm(dim=1).unsqueeze(1)
