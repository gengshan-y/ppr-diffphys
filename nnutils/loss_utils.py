...
def eikonal_loss(mlp, embed, pts, bound, is_init=False):
    """
    ...
    bs = pts.shape[0]
    probs = torch.ones(bs)
    rand_inds = torch.multinomial(probs, 1000, replacement=False)
    pts = pts[rand_inds]
...
def feat_match_loss(nerf_feat, embedding_xyz, feats, pts, pts_prob, bound, 
    ...
    pts_pred, match_unc = feat_match(nerf_feat, embedding_xyz, feats, 
            ...
            #bound,grid_size=8,is_training=is_training, rt_entropy=True,
            #conv3d_net = nerf_feat.conv3d_net)
    pts_pred, match_unc = feat_match(nerf_feat, embedding_xyz, feats, 
