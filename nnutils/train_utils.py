...
from nnutils.geom_utils import lbs, reinit_bones, warp_bw, warp_fw, vec_to_sim3,\
                               ...
                               correct_bones, fid_reindex, K2mat, mat2K, K2inv, \
...
class v2s_trainer():
        ...
        with torch.no_grad():
            ...
            for j in range(0, len(idx_render), chunk):
                ...
                if opts.use_embed:
                    ...
                    rendered_seq['match_unc'][-1] = (rendered_seq['match_unc'][-1] - rendered_seq['match_unc'][-1].min()) /\
                                                     (rendered_seq['match_unc'][-1].max() - rendered_seq['match_unc'][-1].min())
                    rendered_seq['match_unc'][-1] = torch.cat([rendered_seq['match_unc'][-1],
                                                         (rendered_seq['match_unc'][-1] > 0.8).float()],2)
                if opts.use_proj:
            ...
            if opts.local_rank==0:
                ...
                if opts.lbs:
                    ...
                    if opts.pre_skel!="":
                        body_code = self.model.rest_pose_code.weight
                        joint_path = '%s/joint_rest-%02d.obj'%(self.save_dir,suffix_id)
                        rob_path = '%s/rob-rest-%02d.jpg'%(self.save_dir,suffix_id)
                        joint_rest, angle_rest, robot_rendered, rest_robot_mesh = \
                                visualize_joints(self.model, body_code, rob_path)
    ...
    def train(self):
        ...
        if opts.warmup_rootmlp:
            ...
            #depth = 0.1* np.sqrt(self.model.latest_vars['rtk'][:,3,0]*\
            #                            self.model.latest_vars['rtk'][:,3,1]) / opts.render_size
            # use h as heuristic for depth
            depth = 0.1* self.model.latest_vars['rtk'][:,3,1] / opts.render_size
            xytrn = depth[...,None] * \
        ...
        if mesh_dict_in is None:
            ...
            for i in range(0, bs_pts, chunk):
                ...
                if vidid is not None:
                    # expand video id
                    vidid_chunk = vidid * torch.ones(xyz_embedded.shape[:-1]).to(model.device)
                    vidid_chunk = vidid_chunk.long()
                else: vidid_chunk = None
                out_chunks += [model.nerf_coarse(xyz_embedded, sigma_only=True, vidid=vidid_chunk)]
                ...
                if opts.use_cc:
                    ...
                    if is_eval:
                        try: 
                            ...
                            print('not able to smooth the mesh')
                            import subprocess
                            #import optimesh
                            ...
                            print(subprocess.check_output(['./Manifold/build/simplify', '-i', 'tmp/input.obj', '-o', 'tmp/simple.obj', '-m', '-f', '20000']))
