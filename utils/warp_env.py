...
from utils.torch_utils import Embedding, NeRF, clip_grad
...
from nnutils.urdf_utils import articulate_robot_rbrt, articulate_robot_rbrt_batch,\
                               articulate_robot
from nnutils.geom_utils import se3_vec2mat, se3_mat2vec, rot_angle
...
class Scene(nn.Module):
    def __init__(self, opts, dataloader, dt = 5e-4, device='cuda'):
        ...
        self.pose_code = FrameCode(num_freq = opts.num_freq, 
                                   ...
                                   vid_offset = self.data_info['offset'])
        ...
        self.num_bones = self.robot.num_bones
        self.nerf_root_rts = RTExpMLP(self.gt_steps,
                             opts.num_freq,opts.t_embed_dim,self.data_info['offset'],
                             delta=False)
        self.nerf_body_rts = nn.Sequential(self.pose_code,
               SkelHead(urdf=self.robot.urdf,joints=self.robot.joints,
              sim3=self.robot.sim3, rest_angles=self.robot.rest_angles,
                        in_channels_xyz=opts.t_embed_dim,in_channels_dir=0,
                        out_channels=self.robot.num_dofs, raw_feat=True))
        ...
        # robot hyperparams
        density=1000 * density_scale
        kp=220.* density_scale
        kd=2.* density_scale
        # env
        ...
        wp.sim.parse_urdf(urdf_path,
            ...
            floating=True,
            density=density, # collision geometry needs >0 density, but this will ignore urdf mass
            armature=0.01, # additional inertia
            stiffness=220., # ke gain
            damping=2., # kd gain but was set to zero somhow
            shape_ke=1.e+4, # collsion spring/damp/friction
            shape_kd=0,
            shape_kf=1.e+2,
            shape_mu=1.0,
        ...
        self.n_actuators = len(self.articulation_builder.joint_q)-7
        ...
        # torch parameters
        #self.global_q = nn.Parameter( torch.cuda.FloatTensor([0.,1.,0.,0.,0.,0.,1.]) )
        ...
        self.torque_mlp = NeRF(tscale = 1./self.gt_steps, N_freqs = 6, 
                        D=8, W=256,
                        out_channels=6+self.n_actuators,
                        in_channels_xyz=13,
                        )
        
        self.residual_f_mlp = NeRF(tscale = 1./self.gt_steps, N_freqs = 6, 
                        D=8, W=256,
                        out_channels=6*(1+self.n_actuators),
                        in_channels_xyz=13,
                        )
        ...
        # functions
        self.pred_est_q_rect = rectify_func(self.pred_est_q, self.rotate_frame, self.global_q)
        self.pred_est_q_rect_rod = rectify_func(self.pred_est_q, self.rotate_frame, self.global_q, to_quat=False)
    ...
    def pred_est_q(self,steps_fr):
        ...
        pred_mat = self.nerf_root_rts(steps_fr.reshape(-1,1))
        pred_rot = pred_mat[...,:9].view(-1,3,3)
        pred_trn = pred_mat[...,9:].view(-1,3,1)
        pred_mat = torch.cat([pred_rot, pred_trn],2) # -1,3,4
    ...
    def pred_est_ja(self,steps_fr):
        ...
        """
        bs,n_fr = steps_fr.shape
        pose_codes = self.nerf_body_rts[0](steps_fr.reshape(-1,1))
        _,pred_joints = self.nerf_body_rts[1].forward_abs(pose_codes)
    ...
    def reinit_envs(self,num_envs, wdw_length, is_eval=False):
        ...
        if hasattr(self, env_name):
            ...
            self.env.joint_attach_ke = 16000.
            self.env.joint_attach_kd = 200.
    ...
    def preset_data(self, dataloader):
        if hasattr(dataloader, "gtpoints"):
            self.gtpoints = dataloader.gtpoints
        ...
        if hasattr(dataloader, 'amp_info'):
            self.amp_info = dataloader.amp_info
            #TODO clip to first 50 frames
            #self.amp_info = self.amp_info[:50]

        self.samp_int = dataloader.samp_int
        self.data_info = dataloader.data_info

        #self.gt_steps = len(self.data_info['impath'])-2 # for k frames, need to step k-1 times
        self.gt_steps = len(self.amp_info)-1 # for k frames, need to step k-1 times
    ...
    @staticmethod
    def get_batch_input_banmo(banmo_root, banmo_joint, steps_fr):
        """
        steps_fr: bs, T
        target_q:    bs,T,7
        target_ja:  bs,T,dof
    ...
    def forward(self):
        ...
        target_q, target_ja, target_qd, target_jad = \
                            self.get_batch_input(self.amp_info_func,steps_fr, self.in_bullet)
        #TODO replace this with banmo outputs
        #
        #target_q, target_ja, target_qd, target_jad = \
        #                    self.get_batch_input_banmo(self.banmo_root, 
        #                                        self.banmo_joint, steps_fr)

        # transform to ground
        target_q = self.rotate_frame(self.global_q, target_q)
        # rotate velocity vecs
        # only rotate the first 3 elements
        global_qd = self.global_q.clone()
        global_qd[:3] = 0
        target_qd_rev = torch.cat([target_qd[...,3:], target_qd[...,:3]],-1)
        target_qd[...,:3] = self.rotate_frame(global_qd, target_qd)[...,:3]
        target_qd[...,3:] = self.rotate_frame(global_qd, target_qd_rev)[...,:3] 
        # difference between body_q / target_q: body_q is a result of fk
        ...
        #pred_joints = self.pred_est_ja(steps_fr)
        #pred_q      = self.pred_est_q_rect(steps_fr)
        #delta_ja = pred_joints - target_ja
        #delta_q = pred_q - target_q
        #target_q = pred_q
        #ref_ja = pred_joints

        est_q = self.compose_delta(target_q, delta_q)
        #est_q = target_q + delta_q
        ...
        delta_rqd = self.compute_gradient(pred_est_q_comp, steps_fr.clone())
        # need to add another rotation term to account delta root 
        # only rotate the first 3 elements
        delta_q_clone = delta_q.clone()
        delta_q_clone[...,:3] = 0
        target_qd_rev = torch.cat([target_qd[...,3:], target_qd[...,:3]],-1)
        est_rqd_rot = self.rotate_frame(delta_q_clone, target_qd)[...,:3]
        est_rqd_trn = self.rotate_frame(delta_q_clone, target_qd_rev)[...,:3]
        est_rqd = torch.cat([est_rqd_rot, est_rqd_trn],-1)
        ...
        q_init = state_q[0]
        qd_init = state_qd[0]
        body_qs,body_qd = ForwardWarp.apply(q_init, qd_init, torques, res_fin, ref, self)
        ...
        if self.opts.rollout:
            ...
            state_q = state_q[self.frame2step].reshape(self.wdw_length+1, self.num_envs, -1).permute(1,0,2)
            state_qd=state_qd[self.frame2step].reshape(self.wdw_length+1, self.num_envs, -1).permute(1,0,2)
            state_body_q, state_body_qd, self.msm = ForwardKinematics.apply(state_q, 
                                                                 state_qd, self)
        
        # make sure the feet is above the ground
        #TODO 
        mesh_pts,faces_single = articulate_robot_rbrt_batch(self.nerf_body_rts[1].urdf, state_body_q)
        foot_height = mesh_pts[...,1].min(-1)[0] # bs,T
        ...
        if self.opts.rollout:
            ...
            for s in range(self.wdw_length_full):
                if s%100 == 0:
                    pdb.set_trace()
                #msm = state_to_msm(state_q[0,0], state_qd[0,0], foot_pos[0,0], self.in_bullet)
                msm = state_to_msm(state_q[0,s], state_qd[0,s], foot_pos[0,s], self.in_bullet)
        ...
        loss_root = se3_loss(body_traj, body_target).mean(-1)[:,1:]# [...,0] # bs,T, dof => bs,T
        ...
        ## velocity loss
        loss_vel = se3_loss(state_body_qd, target_body_qd).mean(-1)[:,1:]
    ...
    def query(self):
        ...
        x_rest = self.nerf_body_rts[1].urdf
        use_urdf=True 
        for frame in range(len(self.obs)):
            ...
            x_sim = articulate_func(x_rest, obs, in_bullet=self.in_bullet, use_urdf=use_urdf)
...
class ForwardKinematics(torch.autograd.Function):
    ...
    def forward(ctx, rj_q, rj_qd, self):
        ...
        with ctx.tape:
            ...
            for it,step in enumerate(self.frame2step):
                ...
                body_q_sub = wp.to_torch(self.state_steps[step].body_q) # bs*-1,7
                body_q_sub = body_q_sub.reshape(bs,-1,7)
                body_q.append( body_q_sub )
                msm.append(body_q_sub.detach().cpu().numpy()[0] )
                
                body_qd_sub = wp.to_torch(self.state_steps[step].body_qd) # bs*-1,6
                body_qd_sub = body_qd_sub.reshape(bs,-1,6)
                body_qd.append( body_qd_sub )
            body_q = torch.stack(body_q, 1) # bs,T,dofs,7
            body_qd = torch.stack(body_qd, 1)
    ...
    def backward(ctx, adj_body_qs, adj_body_qd, _):
        ...
        if len(rj_q_grad)>0:
            ...
            rj_q_grad = rj_q_grad.view(-1, ctx.bs, ctx.ndof).permute(1,0,2)
        else:
            rj_q_grad = None
        ...
        if len(rj_qd_grad)>0:
            ...
            rj_qd_grad = rj_qd_grad.view(-1, ctx.bs, ctx.ndof-1).permute(1,0,2)
        else:
            rj_qd_grad = None
...
class ForwardWarp(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, q_init, qd_init, torques, res_f, refs, self):
        """
        ...
        with ctx.tape:
            ...
            for step in self.local_steps:
                ...
                self.integrator.simulate(self.env,
                                        self.state_steps[step],
    ...
    def backward(ctx, adj_body_qs, adj_body_qd):
        ...
        try: q_init_grad = wp.to_torch(ctx.tape.gradients[ctx.q_init]).clone()
        except: q_init_grad = None
        
        try: qd_init_grad = wp.to_torch(ctx.tape.gradients[ctx.qd_init]).clone()
        ...
        ctx.tape.zero()
        return (q_init_grad, qd_init_grad, torques_grad, res_f_grad, refs_grad, None)

def can2gym2gl(x_rest, obs, in_bullet=False, use_urdf=False, use_angle=False):
    if use_urdf:
        if use_angle:
            ...
            obs = obs[x_rest.unique_body_idx] # 0,3,...end
            mesh = articulate_robot_rbrt(x_rest, obs)
    else:
        mesh = x_rest.copy()
    return mesh
